...
mod tests {
    ...
    #[test]
    fn extract_endpoint_returns_first_capture_group() {
    ...
    #[test]
    fn extract_endpoint_strips_ansi_before_matching() {
        let pattern = Regex::new(r"Local:\s+(http://[^\s]+)").unwrap();
    ...
    #[test]
    fn extract_endpoint_returns_none_when_pattern_misses() {
        let pattern = Regex::new(r"Local:\s+(http://[^\s]+)").unwrap();
        assert_eq!(extract_endpoint("warning: nothing here", &pattern), None);
    }

    #[test]
    fn extract_endpoint_returns_none_when_pattern_lacks_capture_group() {
        let pattern = Regex::new(r"Local:\s+http://[^\s]+").unwrap();
        // No capture group → captures().get(1) is None.
        assert_eq!(
